...
<body>
 <div class="margin clearfix">
   <div class="border clearfix">
       <span class="l_f">
        <a id="Competence_add" class="btn btn-warning" title="添加角色"><i class="fa fa-plus"></i>添加角色</a>
        <a href="javascript:ovid()" class="btn btn-danger"><i class="fa fa-trash"></i>批量删除</a>
       </span>
       <span class="r_f">共：<b>5</b>类</span>
     </div>
     <div class="compete_list">
       <table id="sample_table" class="table table-striped table-bordered table-hover">
		 <thead>
			<tr>
             ...
             <c:forEach items="${role}" var="item" varStatus="vs">
			  <tr>
				...
				<td>
					<a title="修改权限" href="javascript:;"  onclick="editPermission('${item.id}');" class="btn btn-xs btn-info" ><i class="fa fa-check-circle bigger-120"></i></a>
 ...
 $('#Competence_add').on('click', function(){
     ...
     $(".col-sm-9 input[type$='text'],#form_textarea").each(function(n){
		  ...
		  if(num>0){  return false;}	 	
			  ...
			  $.ajax({
				  url:'${pageContext.request.contextPath}/permission/insertRole',
				  type:'post',
				  data:$("form[name='addRoleForm']").serializeArray(),
				  success:function(val){
                      if(val.code == 1){
						  layer.alert(val.data,{
						   ...
						   layer.close(index);
						  window.location.href="${pageContext.request.contextPath}/permission/selectAllRole";
					  }
					  if(val.code == 2){
						  layer.alert(val.data,{
							  title: '提示框',
